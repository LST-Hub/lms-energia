import { getTaskDataActivityRestletScriptDeploymentId } from "../../../../../src/utils/allActivityNsAPI";
...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === "GET") {
      ...
      const allTaskActivityData =
        await getTaskDataActivityRestletScriptDeploymentId(lid);

        const formatteTaskdData = allTaskActivityData.list.map(
          (item) => item.values
        );
      response({
        res,
