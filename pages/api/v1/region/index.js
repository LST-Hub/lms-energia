...
import { getRegionRestletScriptDeploymentId } from "../../../../src/utils/NsAPIcal";

export default async function handler(req, res) {
  try {
    if (req.method === "GET") {
      const query = `SELECT id, name, fullName FROM Location WHERE isinactive='F'`;
      ...
      const regionData = await getRegionRestletScriptDeploymentId(query);
      response({
        res,
        success: true,
        status_code: 200,
