...
import { getEnquiryByRestletScriptDeploymentId} from "../../../../src/utils/NsAPIcal";

export default async function handler(req, res) {
  try {
    if (req.method === "GET") {
      const query = `SELECT id, name FROM CUSTOMLISTLMS_ENQUIRY_BY WHERE isinactive='F'`;
      ...
      const enquiryByData = await getEnquiryByRestletScriptDeploymentId(query);
      response({
        res,
        success: true,
        status_code: 200,
        data: enquiryByData,
        message: "Enquiry By Fetched successfully",
      });
