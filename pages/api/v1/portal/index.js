...
import {   getPortalRestletScriptDeploymentId} from "../../../../src/utils/NsAPIcal";

export default async function handler(req, res) {
  try {
    if (req.method === "GET") {
      const query = `SELECT id, name FROM CUSTOMLIST_LMS_PORTAL WHERE isinactive='F'`;
      ...
      const leadPortalData = await getPortalRestletScriptDeploymentId(query);
      response({
        res,
        success: true,
        status_code: 200,
        data: leadPortalData,
        message: "Lead Portal Fetched successfully",
      });
