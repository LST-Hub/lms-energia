...
import {  getCampaignRestletScriptDeploymentId, getVisitUpdateRestletScriptDeploymentId } from "../../../../src/utils/NsAPIcal";

export default async function handler(req, res) {
  try {
    if (req.method === "GET") {
      const query = `SELECT id, name FROM CUSTOMLIST_LMS_VISIT_UPDATE WHERE isinactive='F'`;
      ...
      const leadVisitData = await getVisitUpdateRestletScriptDeploymentId(query);
      response({
        res,
        success: true,
        status_code: 200,
        data: leadVisitData,
        message: "Lead Visit Update Fetched successfully",
      });
