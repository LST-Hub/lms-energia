...
import { getTaskCache, getTimesheetSettingsCache, isUserHasAccess } from "../../../../../lib/backendUtils";
import { ApprovalStatus } from "@prisma/client";
import { redisGetTask } from "../../../../../lib/redis-operations";
...
export default async function handler(req, res) {
  try {
    ...
    if (id) {
      ...
      taskTime = await prisma.task.findUnique({
        where: {
    ...
    if (req.method === "GET") {
      ...
      const hasAccess = await isUserHasAccess(
        ...
        perAccessIds.view,
        { projectId: taskCatche.projectId }
      );
      if (!hasAccess) {
        response({
          res,
      ...
      const timesheets = taskTime.timesheets;
      let actualTime = 0;
      timesheets.forEach((timesheet) => {
        actualTime += Number(timesheet.duration);
      });
      const time = { id: taskTime.id, actualTime: actualTime };
    ...
    console.log("error in tasks actual-time file", error);
    response({
      res,
      success: false,
      status_code: 500,
