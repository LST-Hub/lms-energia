...
function WorkingDays({ selectedDayHours, setSelectedDayHours, mode }) {
  ...
  const columns = useMemo(
            ...
            <TkRow>
              <TkCol lg={3}>
                <EditableCell
                  type="text"
                  name="hours"
                  disabled={selectedDayHours[cellProps.row.original.id] === undefined || viewMode}
    ...
    [selectedDayHours, setSelectedDayHours, viewMode]
  );
        ...
        {/* <TkCard> */}
          <TkCardBody className={"pt-0"}>
            <TkTableContainer
              ...
              data={days}
              rowSelection={true}
              onRowSelection={handelDaySelection}
              getRowId={(row) => row.id}
              getInitialSelectedRows={() => {
                let ids = {};
                for (const key in selectedDayHours) {
                  ids[key] = true;
                }
                return ids;
              }}
              disableRowSelectChkBox={viewMode}
            />
          </TkCardBody>
        {/* </TkCard> */}
      </TkCol>
