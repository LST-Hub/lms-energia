...
function TableToolBar({
  ...
  const results = useQueries({
    queries: [
      ...
      // {
      //   queryKey: [RQ.allProjectList, selectedProjectId],
      //   queryFn: tkFetch.get(
      ...
      //   queryFn: tkFetch.get(
      //     `${API_BASE_URL}/task/list${selectedProjectId ? `?projectId=${selectedProjectId}` : ""}${
  ...
  // useEffect(() => {
  //   if (statusFilterDropdownOptions[2].value === "Approved") {
  //     onTimesheetStatusChange(statusFilterDropdownOptions[2]);
  //   }
  // }, []);
              ...
              render={({ field }) => (
                <TkSelect
                  ...
                  onChange={(e) => {
                    console.log("e", e);
                    field.onChange(e);
                    queryClient.invalidateQueries([RQ.allTaskList, selectedProjectId]);
